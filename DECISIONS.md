...
2026-03-15 — Debug hooks gated behind VITE_ENABLE_DEBUG env var — `window.__gameDebug`, `window.__playtestLog`, `window.__exportPlaytestLog`, and `game-observer` DOM element all stripped from student builds. Uses explicit `=== 'true'` check (Vite env vars are strings). Dev server loads `.env.development` automatically. Playwright tests inject via `playwright.config.ts` webServer command. Removes the easiest AI-agent cheating path without attempting real anti-cheat (client-side game, not possible). See `deploy/DISABLE_DEBUG_HOOKS.md` (gitignored).

2026-03-15 — Water stress auto-pause suppressed during watering restriction — Previously, the `water_stress` auto-pause fired with a "Water Field" button even when `wateringRestricted` was true. Player clicked it, nothing happened. Now sends a `water_warning` notification instead. Pause token (`waterStressPausedThisSeason`) resets when restriction expires via `expireActiveEffects`, so the actionable pause fires once the player can irrigate again. Handles overlapping restrictions correctly (only clears when all expire).
